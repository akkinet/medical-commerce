...
const Products = ({ productList, brands }) => {
  ...
  const clearAllFilters = async () => {
    ...
    const res = await fetch(`/api/product?for=all`);
    const data = await res.json();
    setProducts(data.productList);
        ...
        {currentProducts?.length ? currentProducts.map((prod) => (
          <div className="p-2 flex flex-col" key={prod.id}>
            <Link href={`/product/${prod.id}`}>
              <div className="border shadow-xl shadow-teal-200/20 border-teal-200 hover:shadow-teal-300/40 hover:border-2 rounded-lg overflow-hidden h-72 
          ...
          <div className="text-center lg:w-[60vw] lg:h-[50vh] mt-5 flex items-center justify-center animate-pulse">
            <Image src="https://res.cloudinary.com/dduiqwdtr/image/upload/f_auto,q_auto/v1/assets/wlplpvutch0tan3kocst" height={600} width={500} />
          </div>
        )}
      </div>
